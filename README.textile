...
pile up or concurrent execution over many machines. This is mostly used on
cron jobs.
...
using NFS you can not only control if the command should or not run on the
local machine, but you can coordinate execution of that particular command
beetwen a set of machines.
...
lockd running in your client. Sure you have a few different modes to choose,
and some of them does not even need the lockd or NFS server locking support to
work.
